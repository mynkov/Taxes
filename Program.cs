...
var dividendLines = lines2024.Where(x => x.StartsWith("Open Positions,Data,Summary"));


var taxLines = lines.Where(x => x.StartsWith("Trades,Data,Order,Stocks") && (x.Contains("O;") || x.EndsWith(",O")));
...
//httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", authorizationBearerHeader);
        ...
        var currencyCode1 = "840"; // $ USA


Dictionary<string, decimal> dic = new Dictionary<string, decimal>();
Dictionary<string, decimal> dicUsd = new Dictionary<string, decimal>();
...
foreach (var taxLine in taxLines)
{
    ...
    else
    {
        ...
        var description = decimal.Parse(lineItems[13]);
        ...
        var incomeDateResponse = await httpClient.GetStringAsync($"https://lkfl2.nalog.ru/taps/api/v1/dictionary/currency-rates?code={currencyCode1}&date={incomeDateString1}");
        var incomeDateCurrencyRate = JsonConvert.DeserializeObject<CurrencyRate>(incomeDateResponse);

            //  var incomeDateResponse = await httpClient.GetStringAsync($"http://www.cbr.ru/scripts/XML_daily.asp?date_req=18.01.2022");
                
        //var incomeDateCurrencyRate = JsonConvert.DeserializeObject<CurrencyRate>(incomeDateResponse);

             var amount = (decimal)incomeDateCurrencyRate.Rate * description;

        taxes.Add(new Tax(date, ticker, amount, currency, description.ToString(), string.Empty));

        if(!dic.ContainsKey(ticker))
        {
            dic.Add(ticker, amount);
            dicUsd.Add(ticker, description);
        }
else
{
    dic[ticker]+= amount;
    dicUsd[ticker] += description;
}
...
var dic2 = new Dictionary<string, decimal>{
    //{"ITOT", 47000},
   // {"VWO", 39200},
  //  {"VEA", 66200},
   // {"VYM", 20128},
...
foreach(var d in dic.OrderByDescending(x => x.Value))
{
    ...
    var paydUsd = dicUsd[d.Key];

    var currentPrice = dic2.ContainsKey(d.Key) ? dic2[d.Key] : 0M;
